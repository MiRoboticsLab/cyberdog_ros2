...
: cyberdog_decision::CascadeManager("motion_manager", "")
{
...
void MotionManager::checkout_mode()
{
  ...
  uint8_t inter_changed_(cyberdog_decision::DEFAULT);
  ...
  if (automation_manager_node_->get_current_state().id() ==  // NOLINT
    lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE ||
    automation_manager_node_->chainnodes_state_ == cyberdog_decision::PART_DEACTIVE)
  {
    ...
    while (automation_manager_node_->chainnodes_state_ != cyberdog_decision::ALL_DEACTIVE &&
  ...
  while (rclcpp::ok() && new_request) {
    if (mode_server_->is_cancel_requested()) {
      ...
      if (next_mode.control_mode >= Mode_T::MODE_SEMI) {
        ...
        if (automation_manager_node_->get_current_state().id() ==  // NOLINT
          lifecycle_msgs::msg::State::PRIMARY_STATE_ACTIVE ||
          automation_manager_node_->chainnodes_state_ != cyberdog_decision::ALL_DEACTIVE)
        {
          automation_manager_node_->auto_check(cyberdog_utils::CHECK_TO_PAUSE);
        }
      }
    ...
    if (new_request) {
      switch (next_mode.control_mode) {
        ...
        case Mode_T::MODE_MANUAL: {
            ...
            if (condition_pre) {
              if (check_inside) {
                ...
                {
                  if (inter_changed_ == cyberdog_decision::SUCCEED) {
                    message_info(
                      TAG_MODE +
                      std::string("Auto ") +
                      ...
                      std::string(" succeed"));
                    new_request = false;
                  } else if (inter_changed_ == cyberdog_decision::FAILED) {
                    message_info(
                      TAG_MODE +
                      std::string("Auto ") +
                      gait_label_[mode_trig_gait.gait] +
                    ...
                    switch (result.code) {
                      case rclcpp_action::ResultCode::SUCCEEDED: {
                          if (result.result.get()->succeed) {
                            inter_changed_ = cyberdog_decision::SUCCEED;
                          } else {
                            inter_changed_ = cyberdog_decision::FAILED;
                          }
                          break;
                        }
        ...
        case Mode_T::MODE_TRACK: {
            ...
            if (!check_submode) {
              if (automation_manager_node_->chainnodes_state_ == cyberdog_decision::ALL_ACTIVE) {
                message_info(
                  TAG_MODE +
              ...
              if (check_inside) {
                if (this->get_clock()->now() - current_time <=
                  std::chrono::seconds(timeout_manager_))
                {
                  if (inter_changed_ != cyberdog_decision::DEFAULT) {
                    if (inter_changed_ == cyberdog_decision::SUCCEED) {
                      message_info(
                        TAG_MODE +
                ...
                auto client_callback =
                  [&](rclcpp::Client<SubMode_T>::SharedFuture future) {
                    inter_changed_ = cyberdog_decision::DEFAULT;
...
void MotionManager::init()
{
  ...
  automation_manager_node_ =
    std::make_shared<cyberdog_decision::AutomationManager>("multi");
